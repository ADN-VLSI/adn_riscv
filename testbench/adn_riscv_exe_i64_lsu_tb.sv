module adn_riscv_exe_i64_lsu_tb;
  ...
  always @(posedge clk or negedge arst_n) begin
    if (~arst_n) begin
      ...
      if (dmem_pmi_req_o.mreq && dmem_pmi_rsp_i.mgnt) begin
        ...
        dmem_pmi_rsp_i.mresp <=
            (dmem_pmi_req_o.maddr == 64'hFFFF_0000_0000_0000);
  ...
  // Send LSU Operation
  /////////////////////////////////////////////////////////////////////////////////////////////////
  task automatic send_lsu_op(
      input rv_op_t       req_op,
      input logic [63:0]  req_rs1,
      input logic [63:0]  req_rs2,
      input logic [11:0]  req_imm,
      input logic [5:0]   req_rd,
      input logic [63:0]  exp_data,
      input logic [1:0]   exp_sz,
      input bit           exp_flt
  );
    ...
    fork
      ...
      begin
        ...
        $display(
          "[%s] [FAIL] LSU ready_o timed out (DUT stuck busy)! [%0t]",
          test_name,
          $realtime
        );
      end
        ...
        if (valid_o && ready_i) begin
          ...
          if (exp_fifo.size() > 0) begin
            ...
            if (wr_addr_o === exp.exp_rd_addr) begin
              ...
              $display(
                "[%s] [FAIL] rd mismatch! Got: %0d, Exp: %0d [%0t]",
                test_name,
                wr_addr_o,
                exp.exp_rd_addr,
                $realtime
            ...
            if (wr_data_o === exp.exp_wr_data) begin
              ...
              $display(
                "[%s] [FAIL] Data mismatch! Got: 0x%016x, Exp: 0x%016x [%0t]",
                test_name,
                wr_data_o,
                exp.exp_wr_data,
                $realtime
            ...
            // Check 3: Fault
            //------------------------------------------------------------------------------
            if ((mem_fault_o === exp.exp_fault) ||
                (mem_fault_o === 1'bx && !exp.exp_fault)) begin
              ...
              $display(
                "[%s] [FAIL] Fault mismatch! Got: %b, Exp: %b [%0t]",
                test_name,
                mem_fault_o,
                exp.exp_fault,
                $realtime
            ...
            $display(
              "[%s] [FAIL] Unexpected valid_o assertion with empty scoreboard! [%0t]",
              test_name,
              $realtime
            );
  ...
  task automatic run_tc_rst_01();
    ...
    if (!valid_o && (mem_fault_o !== 1'b1)) begin
      ...
      $display(
        "[%s] [FAIL] Reset state invalid! valid_o=%b ready_o=%b fault=%b [%0t]",
        test_name,
        valid_o,
        ready_o,
        mem_fault_o,
  ...
  // Check PMI Strobe
  /////////////////////////////////////////////////////////////////////////////////////////////////
  task automatic check_mstrb(
      input rv_op_t      req_op,
      input logic [63:0] req_rs1,
      input logic [11:0] req_imm,
      input logic [7:0]  exp_strb
  );
    ...
    if (dmem_pmi_req_o.mstrb === exp_strb) begin
      ...
      $display(
        "[%s] [FAIL] mstrb mismatch! Got=%b Exp=%b [%0t]",
        test_name,
        dmem_pmi_req_o.mstrb,
        exp_strb,
        $realtime
  ...
  task automatic run_tc_aq_rl_01();
    ...
    if ((dmem_sideband_o.aq === 1'b0) &&
        (dmem_sideband_o.rl === 1'b0)) begin
      note_case(1);
    end else begin
      note_case(0);
      $display(
        "[%s] [FAIL] AQ/RL mismatch for 00 [%0t]",
        test_name,
        $realtime
      );
    end
    ...
    if ((dmem_sideband_o.aq === 1'b1) &&
        (dmem_sideband_o.rl === 1'b0)) begin
      note_case(1);
    end else begin
      note_case(0);
      $display(
        "[%s] [FAIL] AQ/RL mismatch for 10 [%0t]",
        test_name,
        $realtime
      );
    end
    ...
    if ((dmem_sideband_o.aq === 1'b0) &&
        (dmem_sideband_o.rl === 1'b1)) begin
      note_case(1);
    end else begin
      note_case(0);
      $display(
        "[%s] [FAIL] AQ/RL mismatch for 01 [%0t]",
        test_name,
        $realtime
      );
    end
    ...
    if ((dmem_sideband_o.aq === 1'b1) &&
      ...
      note_case(0);
      $display(
        "[%s] [FAIL] AQ/RL mismatch for 11 [%0t]",
        test_name,
        $realtime
      );
    end
    ...
    fork
      ...
      begin
        ...
        if (valid_o === 1'b1) begin
          ...
          if (debug)
            $display(
              "[%s] [PASS] valid_o held during backpressure [%0t]",
              test_name,
              $realtime
            );
          ...
          $display(
            "[%s] [FAIL] valid_o dropped during output backpressure [%0t]",
            test_name,
            $realtime
          );
    ...
    fork
      ...
      begin
        ...
        if (dmem_pmi_req_o.mreq) begin
          ...
          $display(
            "[%s] [FAIL] PMI request not generated during grant backpressure [%0t]",
            test_name,
            $realtime
          );
        end
  ...
  task automatic run_tc_reset_trans_01();
    ...
    if (!valid_o && !dmem_pmi_req_o.mreq) begin
      ...
      if (debug)
        $display(
          "[%s] [PASS] LSU cleared during reset [%0t]",
          test_name,
          $realtime
        );
      ...
      $display(
        "[%s] [FAIL] LSU did not clear during reset! valid_o=%b mreq=%b [%0t]",
        test_name,
        valid_o,
        dmem_pmi_req_o.mreq,
        $realtime
  ...
  task automatic run_tc_invalid_op_01();
    ...
    if (!dmem_pmi_req_o.mreq && !valid_o) begin
      ...
      if (debug)
        $display(
          "[%s] [PASS] Invalid opcode correctly blocked [%0t]",
          test_name,
          $realtime
        );
      ...
      $display(
        "[%s] [FAIL] Invalid opcode generated transaction! mreq=%b valid_o=%b [%0t]",
        test_name,
        dmem_pmi_req_o.mreq,
        valid_o,
        $realtime
